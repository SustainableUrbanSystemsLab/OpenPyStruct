...
{
    /// <summary>Name -> (force factor, length factor) applied to N, V, M and displacements.</summary>
    private static readonly (string Name, double F, double L)[] Units =
    {
        ("N, m", 1.0, 1.0), ("kN, m", 1e-3, 1.0), ("kN, mm", 1e-3, 1e3),
    };
    ...
    protected override void RegisterInputParams(GH_InputParamManager pm)
    {
        ...
        pm.AddParameter(new GH_DropdownParam(Units.Select(u => u.Name), "Units", "Units",
            "Output units for forces, moments and displacements. The engine works in N and m; kN and mm are what a "
            + "drawing reads in. I stays in m⁴.", this, defaultItem: Units[0].Name));
        pm[2].Optional = true;
    }
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pm)
    {
        ...
        pm.AddNumberParameter("Axial", "N", "Axial force per element, in the chosen force unit.", GH_ParamAccess.list);
        pm.AddNumberParameter("Shear start", "Vi", "Shear at the element start.", GH_ParamAccess.list);
        pm.AddNumberParameter("Shear end", "Vj", "Shear at the element end.", GH_ParamAccess.list);
        pm.AddNumberParameter("Moment start", "Mi", "Bending moment at the element start (force × length unit).", GH_ParamAccess.list);
        pm.AddNumberParameter("Moment end", "Mj", "Bending moment at the element end.", GH_ParamAccess.list);
        pm.AddVectorParameter("Displacements", "Disp", "Nodal displacement as a world vector, in the chosen length unit.", GH_ParamAccess.list);
    ...
    protected override void Solve(IGH_DataAccess da)
    {
        ...
        var unit = Units.FirstOrDefault(u => u.Name == this.Selected(2, Units[0].Name));
        if (unit.Name == null) unit = Units[0];
        double F = unit.F, L = unit.L;
        var res = run.Result;
        ...
        da.SetDataList(1, c.Axial.Select(x => x * F));
        da.SetDataList(2, c.ShearI.Select(x => x * F));
        da.SetDataList(3, c.ShearJ.Select(x => x * F));
        da.SetDataList(4, c.MomentI.Select(x => x * F * L));
        da.SetDataList(5, c.MomentJ.Select(x => x * F * L));
        if (run.Model != null)
        {
            da.SetDataList(6, c.Displacements.Select(d => run.Model.ToWorld(d[0], d[1]) * L));
        }
        else
        {
            da.SetDataList(6, c.Displacements.Select(d => new Vector3d(d[0], 0, d[1]) * L));
